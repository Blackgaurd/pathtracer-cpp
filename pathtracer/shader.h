...
// defining in string to make it easier to import
const std::string REALTIME_SHADER = R"shader(#version 330
...
const std::string ONETIME_SHADER = R"shader(#version 330
...
void main() {
    ...
    for (int i = 0; i < render_samples; i++) {
        vec3 ray_d = camera_ray(seed);
        color += trace(camera.pos, ray_d, render_depth, seed) / render_samples;
    }
