...
int main() {
    ...
    objects.push_back(std::make_shared<sphere_t>(sphere_t({0, 0, 0}, 2, color::yellow)));
    objects.push_back(std::make_shared<sphere_t>(sphere_t({0, -10002, 0}, 10000, color::white)));

    std::vector<std::shared_ptr<light_t>> lights;
    lights.push_back(std::make_shared<point_light_t>(point_light_t({-4, 0.5, 2}, 1, color::white, 1)));
